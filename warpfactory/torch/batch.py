...
class TorchMetricBatch:
    ...
    def calculate_metrics(
        ...
        for i in range(batch_size):
            batch_params = {k: v[i].to(self.device) for k, v in params.items()}

            metric = self.solver.calculate_alcubierre_metric(x, y, z, t, **batch_params)
    ...
    def calculate_metrics_parallel(
        ...
        r = torch.sqrt((x - x_s.unsqueeze(1)) ** 2 + y**2 + z**2)

        # Shape function
        f = torch.exp(
            -params["sigma"].unsqueeze(1) * r**2 / params["R"].unsqueeze(1) ** 2
        )

        # Add cutoff for numerical stability
        f = torch.where(r > 5 * params["R"].unsqueeze(1), torch.zeros_like(f), f)
        ...
        return [
            {"g_tt": g_tt[i], "g_tx": g_tx[i], "g_xx": g_xx[i]}
            for i in range(batch_size)
        ]
