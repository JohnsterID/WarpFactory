...
class TorchFieldVisualizer:
    ...
    def plot_scalar_field(
        self, field: torch.Tensor, x: torch.Tensor, y: torch.Tensor
    ) -> plt.Figure:
        """Plot scalar field.
    ...
    def plot_vector_field(
        self, vectors: torch.Tensor, x: torch.Tensor, y: torch.Tensor
    ) -> plt.Figure:
        """Plot vector field.
        ...
        skip = 4
        ax.quiver(
            X[::skip, ::skip], Y[::skip, ::skip], vx[::skip, ::skip], vy[::skip, ::skip]
        )
        ax.set_aspect("equal")
    ...
    def animate_field_evolution(
        ...
        def update(frame):
            ...
            field_next = field_np - dt * (
                vx * np.gradient(field_np, axis=1) + vy * np.gradient(field_np, axis=0)
            )
