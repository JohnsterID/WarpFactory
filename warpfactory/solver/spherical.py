...
class SphericalCurvature:
    ...
    def christoffel(self, metric: Dict[str, np.ndarray],
    ...
    def ricci_tensor(self, metric: Dict[str, np.ndarray],
                     coords: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        r = np.asarray(coords["r"], dtype=float)
        theta = np.asarray(coords.get("theta", np.full_like(r, np.pi / 2)))
        A, B, C, dA, dB, dC, d2A, d2C = self._profiles(metric, r)

        R_tt = (d2A / (2 * B) + dA * dC / (2 * B * C)
                - dA * dB / (4 * B**2) - dA**2 / (4 * A * B))
        R_rr = (-d2C / C + dC**2 / (2 * C**2) + dB * dC / (2 * B * C)
                - d2A / (2 * A) + dA * dB / (4 * A * B) + dA**2 / (4 * A**2))
        R_thth = (1 - d2C / (2 * B) + dB * dC / (4 * B**2)
                  - dA * dC / (4 * A * B))
    ...
    def ricci_scalar(self, metric: Dict[str, np.ndarray],
        ...
        r = np.asarray(coords["r"], dtype=float)
        A, B, C, dA, dB, dC, d2A, d2C = self._profiles(metric, r)
        return (2 / C - 2 * d2C / (B * C) + dC**2 / (2 * B * C**2)
                + dB * dC / (B**2 * C) - d2A / (A * B)
                - dA * dC / (A * B * C) + dA * dB / (2 * A * B**2)
                + dA**2 / (2 * A**2 * B))
    ...
    def kretschmann(self, metric: Dict[str, np.ndarray],
        ...
        r = np.asarray(coords["r"], dtype=float)
        A, B, C, dA, dB, dC, d2A, d2C = self._profiles(metric, r)
        return (4 / C**2 - 2 * dC**2 / (B * C**3) + 2 * d2C**2 / (B**2 * C**2)
                - 2 * dC**2 * d2C / (B**2 * C**3) + 3 * dC**4 / (4 * B**2 * C**4)
                - 2 * dB * dC * d2C / (B**3 * C**2) + dB * dC**3 / (B**3 * C**3)
                + dB**2 * dC**2 / (2 * B**4 * C**2) + d2A**2 / (A**2 * B**2)
                + dA**2 * dC**2 / (2 * A**2 * B**2 * C**2)
                - dA * d2A * dB / (A**2 * B**3) + dA**2 * dB**2 / (4 * A**2 * B**4)
                - dA**2 * d2A / (A**3 * B**2) + dA**3 * dB / (2 * A**3 * B**3)
                + dA**4 / (4 * A**4 * B**2))
