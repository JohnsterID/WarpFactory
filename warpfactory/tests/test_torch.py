...
def test_torch_metric_solver(device, spatial_grid_torch, metric_params):
    ...
    assert torch.isclose(
        components["g_tt"][far_index], torch.tensor(-1.0, device=device)
    )
    assert torch.isclose(
        components["g_tx"][far_index], torch.tensor(0.0, device=device)
    )
    assert torch.isclose(
        components["g_xx"][far_index], torch.tensor(1.0, device=device)
    )
