...
class TestChangeTensorIndex:
    def test_metric_inverse_roundtrip(self):
        metric = alcubierre_metric((1, 8, 8, 8), centered_world((1, 8, 8, 8)),
                                   v=0.5, R=2.0, sigma=2.0)
    ...
    def test_stress_energy_requires_metric(self):
        T = SpacetimeTensor(tensor=np.zeros((4, 4, 1, 4, 4, 4)),
                            type="stress-energy", index="contravariant")
        with pytest.raises(ValueError):
    ...
    def test_stress_energy_roundtrip_through_all_indices(self):
        ...
        chain = ["covariant", "mixedupdown", "contravariant",
                 "mixeddownup", "contravariant"]
        current = T
        for index in chain:
...
class TestGridSolver:
    ...
    def test_schwarzschild_is_vacuum(self):
        ...
        worst = max(np.abs(T.tensor[mu, nu, 0][exterior]).max()
    ...
    def test_time_dependent_grid(self):
        ...
        metric = alcubierre_metric(grid_size, world_center, v=0.5, R=2.0,
                                   sigma=2.0, grid_scale=(0.5, 0.5, 0.5, 0.5))
        T = GridSolver(order=4).solve(metric)
...
class TestThreePlusOne:
    ...
    def test_decomposer_recovers_adm_variables(self):
        ...
        alpha2, beta_down, gamma_down, beta_up, gamma_up = \
...
class TestInterpolation:
    def test_trilinear_exact_on_linear_field(self):
        i, j, k = np.meshgrid(np.arange(5), np.arange(5), np.arange(5),
                              indexing="ij")
    ...
    def test_quadrilinear_exact_on_linear_field(self):
        t, i, j, k = np.meshgrid(np.arange(4), np.arange(4), np.arange(4),
    ...
    def test_legendre_radial_exact_on_cubic(self):
        ...
        r = 7.3
        assert legendre_radial_interp(profile, r) == pytest.approx(
            r**3 - 2*r**2 + 5)
...
class TestGridMetrics:
    def test_alcubierre_flat_far_from_bubble(self):
        ...
        metric = alcubierre_metric(grid_size, world_center, v=2.0, R=2.0,
                                   sigma=4.0)
    ...
    def test_alcubierre_comoving_shift_vanishes_inside(self):
        ...
        metric = alcubierre_comoving_metric(grid_size,
                                            centered_world(grid_size),
                                            v=2.0, R=2.0, sigma=4.0)
        center = metric.tensor[:, :, 0, 16, 8, 8]
    ...
    def test_comoving_builders_reject_multiple_time_slices(self):
        ...
        builders = [
            lambda: alcubierre_comoving_metric(grid_size, world_center,
                                               1.0, 2.0, 2.0),
            lambda: lentz_comoving_metric(grid_size, world_center, 1.0),
            lambda: van_den_broeck_comoving_metric(grid_size, world_center,
                                                   1.0, 2.0, 2.0, 4.0, 2.0, 0.5),
            lambda: modified_time_comoving_metric(grid_size, world_center,
                                                  1.0, 2.0, 2.0, 2.0),
    ...
    def test_van_den_broeck_volume_expansion(self):
        ...
        A = 0.5
        metric = van_den_broeck_metric(grid_size, centered_world(grid_size),
                                       v=1.0, R1=2.0, sigma1=4.0, R2=4.0,
                                       sigma2=4.0, A=A)
        assert metric.tensor[1, 1, 0, 8, 8, 8] == pytest.approx(
            (1 + A)**2, abs=1e-3)
    ...
    def test_modified_time_lapse_modification(self):
        grid_size = (1, 17, 17, 17)
        A = 2.0
        metric = modified_time_metric(grid_size, centered_world(grid_size),
                                      v=0.0, R=2.0, sigma=4.0, A=A)
        # v = 0: g_tt = -((1-fs) + fs/A)^2 -> -1/A^2 inside, -1 outside.
        assert metric.tensor[0, 0, 0, 8, 8, 8] == pytest.approx(
            -1.0/A**2, abs=1e-3)
...
class TestWarpShell:
    def test_shell_metric_structure(self):
        ...
        metric = warp_shell_comoving_metric(
            grid_size, world_center, m=0.1, R1=4.0, R2=8.0, Rbuff=0.5,
            sigma=2.0, smooth_factor=10, v_warp=0.5, do_warp=True,
            r_sample_res=10**4)
        g = metric.tensor
    ...
    def test_shell_without_warp_has_no_shift(self):
        ...
        metric = warp_shell_comoving_metric(
            grid_size, centered_world(grid_size), m=0.05, R1=3.0, R2=6.0,
            r_sample_res=10**4)
    ...
    def test_shell_energy_density_positive_in_shell(self):
        ...
        metric = warp_shell_comoving_metric(
            grid_size, world_center, m=0.1, R1=4.0, R2=8.0, Rbuff=0.5,
            sigma=2.0, smooth_factor=20, r_sample_res=10**4)
        T = GridSolver(order=4).solve(metric)
        assert np.all(np.isfinite(T.tensor))
