...
def test_finite_difference():
    ...
    profile = np.sin(2*x)
    exact = 2*np.cos(2*x)
    err2 = np.max(np.abs(FiniteDifference(order=2).derivative1(profile, x)[2:-2] - exact[2:-2]))
    err4 = np.max(np.abs(FiniteDifference(order=4).derivative1(profile, x)[2:-2] - exact[2:-2]))
    assert err4 < err2 / 10
...
def test_christoffel_symbols():
    ...
    gamma = christoffel.calculate(schwarzschild_metric(r, theta),
                                  {"r": r, "theta": theta})
    ...
    assert np.allclose(gamma["theta_phiphi"], -np.sin(theta)*np.cos(theta), atol=1e-12)
...
def test_ricci_tensor():
    ...
    r, theta = schwarzschild_grid()
    R_munu = ricci.calculate(schwarzschild_metric(r, theta),
                             {"r": r, "theta": theta})
...
def test_ricci_scalar():
    ...
    r, theta = schwarzschild_grid()
    R = ricci_scalar.calculate(schwarzschild_metric(r, theta),
                               {"r": r, "theta": theta})
