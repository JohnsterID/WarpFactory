...
def test_tensor_plotter():
    ...
    metric = {"g_tt": -(1 - (v * f) ** 2), "g_tx": -v * f, "g_xx": np.ones_like(r)}
...
def test_slice_data():
    ...
    fig = slicer.plot_slice(slice_xz, x, z, title="XZ Slice", xlabel="X", ylabel="Z")
    assert isinstance(fig, plt.Figure)
    plt.close(fig)

    # Plot with custom colormap
    fig = slicer.plot_slice(slice_yz, y, z, title="YZ Slice", cmap="viridis")
    assert isinstance(fig, plt.Figure)
    plt.close(fig)
