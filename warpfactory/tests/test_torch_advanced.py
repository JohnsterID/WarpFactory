...
def test_metric_batch_processing(device, spatial_grid_torch, batch_params):
    ...
    for metric in metrics:
        ...
        assert torch.isclose(
            metric["g_tt"][far_index], torch.tensor(-1.0, device=device)
        )
        assert torch.isclose(
            metric["g_tx"][far_index], torch.tensor(0.0, device=device)
        )
        assert torch.isclose(
            metric["g_xx"][far_index], torch.tensor(1.0, device=device)
        )
...
def test_field_visualization(device, spatial_grid_torch):
    ...
    vectors = torch.stack([-x * field, -y * field, -z * field], dim=0)
