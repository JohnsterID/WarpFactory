...
def _world_coordinates(
    grid_size: GridSize, world_center: Vector4, grid_scale: Vector4
) -> Tuple[np.ndarray, ...]:
...
def _base_metric(
    name: str, grid_scale: Vector4, params: dict, tensor: np.ndarray
) -> SpacetimeTensor:
    return SpacetimeTensor(
        ...
        index="covariant",
        coords="cartesian",
        scaling=tuple(float(s) for s in grid_scale),
        name=name,
        params=params,
    )
...
def _require_single_time_slice(grid_size: GridSize, name: str) -> None:
    if grid_size[0] != 1:
        raise ValueError(f"{name} requires a single time slice (grid_size[0] == 1)")


def minkowski_metric(
    grid_size: GridSize, grid_scale: Vector4 = (1, 1, 1, 1)
) -> SpacetimeTensor:
...
def alcubierre_comoving_metric(
    grid_size: GridSize,
    world_center: Vector4,
    v: float,
    R: float,
...
def _lentz_warp_factors(
    ...
    regions = [
        ...
        (
            (x_b > 0) & (x_b <= scale) & (x_b + scale > y_b) & (-y_b + scale < x_b),
            0.0,
            1.0,
        ),
        (
        ...
        (
            (x_b > -scale) & (x_b <= 0) & (x_b + scale <= y_b) & (-y_b + scale >= x_b),
            1.0,
            0.0,
        ),
...
def van_den_broeck_comoving_metric(
    grid_size: GridSize,
    world_center: Vector4,
    v: float,
    R1: float,
...
def modified_time_comoving_metric(
    grid_size: GridSize,
    world_center: Vector4,
    v: float,
    R: float,
...
def schwarzschild_metric(
    ...
    return _base_metric(
        "Schwarzschild",
        grid_scale,
        {"grid_size": tuple(grid_size), "world_center": tuple(world_center), "rs": rs},
        g,
    )
