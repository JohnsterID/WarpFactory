...
class TensorPlotter:
    ...
    def plot_component(
        ...
        fig, ax = plt.subplots()
        im = ax.pcolormesh(
            X, Y, metric[component], cmap=self.cmaps.redblue(), shading="auto"
        )
    ...
    def plot_heatmap(
        self, metric: Dict[str, np.ndarray], x: np.ndarray, y: np.ndarray
    ) -> plt.Figure:
        ...
        for i, comp1 in enumerate(components):
            for j, comp2 in enumerate(components):
                ...
                if i <= j:  # Upper triangular part
                    im = ax.pcolormesh(
                        metric[comp1], cmap=self.cmaps.redblue(), shading="auto"
                    )
                    plt.colorbar(im, ax=ax)
    ...
    def plot_eigenvalues(
        ...
        X, Y = np.meshgrid(x, y)
        im1 = ax1.pcolormesh(
            X, Y, eigenvals[:, :, 0].real, cmap=self.cmaps.redblue(), shading="auto"
        )
        ...
        im2 = ax2.pcolormesh(
            X, Y, eigenvals[:, :, 1].real, cmap=self.cmaps.redblue(), shading="auto"
        )
