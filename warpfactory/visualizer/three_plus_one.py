...
class ThreePlusOnePlotter:
    ...
    def plot_lapse(self, decomp: Dict, x: np.ndarray, y: np.ndarray) -> plt.Figure:
        ...
        fig, ax = plt.subplots()
        im = ax.pcolormesh(
            X, Y, decomp["alpha"], cmap=self.cmaps.warp(), shading="auto"
        )
    ...
    def plot_shift(self, decomp: Dict, x: np.ndarray, y: np.ndarray) -> plt.Figure:
        ...
        im = ax.pcolormesh(X, Y, beta_mag, cmap=self.cmaps.warp(), shading="auto")
        plt.colorbar(im, ax=ax)
    ...
    def plot_spatial_metric(
        self, decomp: Dict, x: np.ndarray, y: np.ndarray
    ) -> plt.Figure:
        ...
        # Plot gamma_xx
        im1 = ax1.pcolormesh(
            X, Y, decomp["gamma"]["xx"], cmap=self.cmaps.redblue(), shading="auto"
        )
        ...
        # Plot gamma_xy
        im2 = ax2.pcolormesh(
            X, Y, decomp["gamma"]["xy"], cmap=self.cmaps.redblue(), shading="auto"
        )
        ...
        # Plot gamma_yy
        im3 = ax3.pcolormesh(
            X, Y, decomp["gamma"]["yy"], cmap=self.cmaps.redblue(), shading="auto"
        )
